...
interface BudgetBreakdownProps {
  ...
  activeStatus?: "Pending" | "Upcoming" | "Past" | "Rejected" // Add activeStatus prop
}
...
const BudgetBreakdown: React.FC<BudgetBreakdownProps> = ({
  ...
  const canAddServices = (userRole === 'organizer' || userRole === 'vendor') && !isPastEvent
  ...
  const updateServiceEntry = (id: number, field: 'name' | 'cost', value: string) => {
  ...
  const handleAddService = () => {
    ...
    const validEntries = serviceEntries.filter(entry => entry.name.trim() && entry.cost.trim())
      ...
      >
        ...
        {isPastEvent && (
          <div className="mt-2 text-xs text-gray-500 italic">
            This event has already passed. Budget information cannot be modified.
          </div>
        )}
      </div>
                ...
                <div key={entry.id} className="mb-6">
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      value={entry.name}
                      onChange={(e) => updateServiceEntry(entry.id, 'name', e.target.value)}
                    />
                  </div>
                  ...
                  <div className="mb-2">
                    ...
                    <input
                      ...
                      value={entry.cost}
                      onChange={(e) => updateServiceEntry(entry.id, 'cost', e.target.value)}
                    />
                  </div>
                </div>
              ))}
              ...
              <div className="space-y-3 mb-6">
                ...
                {serviceEntries.length > 1 && (
                  <button
                    ...
                    onClick={() => removeServiceEntry(serviceEntries[serviceEntries.length - 1].id)}
                  >
