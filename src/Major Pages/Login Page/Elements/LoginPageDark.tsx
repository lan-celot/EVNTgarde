...
const LoginPageDark: React.FC<{ login: () => void }> = ({ login }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 5 failed attempts, generic message
  ...
  return (
    ...
    <div className="flex w-[1440px] h-[650px] bg-blue-600 rounded-xl shadow-lg overflow-hidden font-poppins">
      ...
      <div className="w-3/5 bg-gray-800 p-10 flex flex-col justify-center rounded-l-[50px] shadow-md">
        <h2 className="text-4xl font-bold text-white mb-4">Log In</h2>
        {error && <p className="text-red-500">{error}</p>}
        <form className="flex flex-col space-y-4"  onSubmit={handleLogin}>
          <label className="text-sm text-white">Enter your email*</label>
          <input placeholder="Email" type="email" className="border p-3 rounded-md text-white" value={email} onChange={(e) => setEmail(e.target.value)} required />
          <label className="text-sm text-white">Enter your password*</label>
          <div className="relative">
          <input placeholder="Password" type={showPassword ? "text" : "password"}  className="border p-3 w-full rounded-md text-white" value={password} onChange={(e) => setPassword(e.target.value)} required />
          ...
          <div className="text-center text-sm text-white">
            Don't have an account? <a href="#" className="text-blue-600" onClick={(e) => { e.preventDefault(); navigate("/role-selection-dark"); }}>Sign up</a>
          </div>
        </form>
      </div>
    </div>
