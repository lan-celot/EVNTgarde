...
router.post('/uploadGuestCSV', upload.single('file'), async (req: Request, res: Response) => {
  const typedReq = req as MulterRequest;
  const eventId = parseInt(typedReq.body.eventId, 10);
  const filePath = typedReq.file?.path;

  if (!eventId || !filePath) {
    return res.status(400).json({ success: false, message: 'Missing eventId or file' });
  }
  ...
  fs.createReadStream(filePath)
    ...
    .on('end', async () => {
      ...
      try {
        ...
        await client.query('COMMIT');
        res.status(201).json({ success: true, message: 'CSV guests added successfully' });
      } catch (err: any) {
        await client.query('ROLLBACK');
